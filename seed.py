...
def seed():
    ...
    with app.app_context():
        ...
        db.session.add_all([admin, demo, lionel])
        db.session.commit()
        print(f'[seed] Created {User.query.count()} users')

        # ── Items (sesuai frontend mock data) ──────────────────────────────
        now = datetime.utcnow()
        ...
        for d in items:
            mins = d.pop('minutes')
            it = Item(**d,
                      start_price=Decimal(str(d.get('start_price', 0))),
                      current_bid=Decimal(str(d.get('current_bid', 0))),
